...
import { getSliderById, updateSlider, deleteSlider } from "../../../../lib/sliders-data"

// Import dữ liệu từ route chính - cập nhật để khớp với dữ liệu mới
const sliders = [
  {
    id: 1,
...
export async function PUT(request, { params }) {
  try {
    ...
    const updatedSlider = updateSlider(id, body)
    ...
    return NextResponse.json({ error: "Lỗi khi cập nhật slider" }, { status: 500 })
  }
}
...
export async function DELETE(request, { params }) {
  try {
    const id = Number.parseInt(params.id)
    const success = deleteSlider(id)

    if (!success) {
      return NextResponse.json({ error: "Không tìm thấy slider" }, { status: 404 })
    }

    return NextResponse.json({ message: "Xóa slider thành công" })
  } catch (error) {
    console.error("DELETE Error:", error)
    return NextResponse.json({ error: "Lỗi khi xóa slider" }, { status: 500 })
  }
}

// Export sliders để có thể import từ route chính
export { sliders }
